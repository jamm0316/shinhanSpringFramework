...
import com.shinhan.myapp.model.util.DateUtil.DateUtil;
...
public class EmpDAO implements EmpDAOInterface {
    ...
    public List<JobDTO> selectAllJobs() {
        String sql_selectAllJobs = "select * from jobs";

        Connection conn = null; // 지역 변수로 변경
        PreparedStatement st = null; // PreparedStatement 사용
        ResultSet rs = null;
        List<JobDTO> joblist = new ArrayList<>();
        try {
            conn = ds.getConnection();
            st = conn.prepareStatement(sql_selectAllJobs);
            rs = st.executeQuery();
            while (rs.next()) {
                JobDTO job = makeJobs(rs);
                joblist.add(job);
                //StringBuilder로 만들어보기
            }
    ...
    private JobDTO makeJobs(ResultSet rs) throws SQLException {
    ...
    public List<EmpDTO> selectByCondition(Map<String, Object> map) {
        ...
        try {
            ...
            st.setString(3, (String) map.get("jobid"));  //?에 값을 채우기
            st.setString(4, (String) map.get("jobid"));  //?에 값을 채우기
            st.setDouble(5, Double.parseDouble(str_sal));  //?에 값을 채우기
    ...
    public int insert(EmpDTO emp) {
        ...
        try {
            ...
            //st.setInt에는 null값을 넣지 못함. => setObject로 넣어주어야함.
